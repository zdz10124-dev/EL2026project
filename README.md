...
## 一、today_eat_app — Flutter 移动应用（食动健康）

运动、饮食与恢复一体化健康应用。支持餐食和运动记录、运动截图 AI 识别、7/30 天综合健康分析，同时保留原有吃什么、日记、营养分析、偏好分析和联网推荐功能。
...
| 🏃 运动记录 | 手动填写或上传运动 App 截图识别时长、距离、心率等信息 |
| ❤️ 综合健康 | 联合近 7/30 天饮食与运动指标，生成带依据的饮食、运动和休息建议 |
| 🗓️ 统一时间线 | 饮食与运动按时间合并展示，支持分类筛选、编辑和删除 |
| 📤 数据导出 | 将健康档案、饮食和运动记录导出为 JSON 文件 |
...
| POST | `/v1/ai/chat` | 通用 AI 代理转发 |
| POST | `/v1/ai/exercise-recognition` | 运动截图结构化识别 |
| POST | `/v1/ai/health-analysis` | 饮食运动综合健康分析 |
